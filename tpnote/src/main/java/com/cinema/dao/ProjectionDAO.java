...
public class ProjectionDAO extends CinemaDAO<ProjectionDto> {
	...
	public List<ProjectionDto> findAll() {
		...
		try {
			...
			PreparedStatement st =CinemaDAO.cn.prepareStatement("Select id,date,nb_place_libre,tarif from projection");
			...
			while (result.next()) {
				...
				ProjectionDto projection = new ProjectionDto(id,date,nb_place_libre,tarif);
	...
	public ProjectionDto findById(int id) {
		...
		try {
			PreparedStatement st =CinemaDAO.cn.prepareStatement("Select id,date,nb_place_libre,tarif from projection where id=?");
			st.setInt(1, id);
			ResultSet result = st.executeQuery();
	...
	public boolean save(ProjectionDto obj) {
		try {
			...
			if (obj.getId()==-1) {
				st =CinemaDAO.cn.prepareStatement("insert into projection (date,nb_place_libre,tarif) values (?,?,?)");
			}
			else {
				st =CinemaDAO.cn.prepareStatement("insert into projection (date,nb_place_libre,tarif,id) values (?,?,?,?)");
	...
	public boolean update(int id,ProjectionDto obj) {
		try {
			PreparedStatement st =CinemaDAO.cn.prepareStatement("update projection set id=?,date=?,nb_place_libre=?,tarif=? where id=?");
